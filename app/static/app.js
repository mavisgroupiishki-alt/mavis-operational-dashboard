...
const PROD_LABELS={
  ...
  dormant_all_with_reason_count:["Зависшие с причиной — всего","num"],dormant_with_reason_count:["Зависшие с причиной — период","num"]
};
...
function renderExperts(){
  ...
  const cards=experts().map(e=>{const nm=expertNpsMeta(e),nv=nm.value;return `<div class="expert-card"><div class="expert-card-top"><div><div class="expert-name">${esc(e.name)}</div><div class="expert-result">${money(e.closed_amount)}</div></div><button class="btn nps-button" data-nps-edit="1" data-expert="${attr(e.name)}">${nv===null?'+ Добавить NPS':'NPS '+fmt(nv)+' оц. · добавить'}</button></div><div class="expert-stats"><div><span>Закрыто</span><strong>${tdLink(e.closed_count,"production","closed_count","num",{expert:e.name})}</strong></div><div><span>В работе</span><strong>${tdLink(e.active_count,"production","active","num",{expert:e.name})}</strong></div><div><span>В норме</span><strong>${pct(e.within_norm_pct)}</strong></div><div><span>Возвраты</span><strong>${tdLink(e.returns_count,"production","returns_count","num",{expert:e.name})}</strong></div></div><details class="nested"><summary>По продуктам (${e.products.length})</summary><div class="nested-body"><table><thead><tr><th>Продукт</th><th class="num">Закрыто</th><th class="num">План, шт</th><th class="num">% плана</th><th class="num">Сумма</th><th class="num">Срок</th><th class="num">В норме</th><th></th></tr></thead><tbody>${e.products.map(p=>{const plan=expertProductPlan(e.name,p.name);return `<tr><td>${esc(p.name)}</td><td class="num">${tdLink(p.closed_count,"production","closed_count","num",{expert:e.name,product:p.name})}</td><td class="num">${plan?fmt(plan):"—"}</td><td class="num">${plan?pct(p.closed_count/plan*100):"—"}</td><td class="num">${tdLink(p.closed_amount,"production","closed_amount","money",{expert:e.name,product:p.name})}</td><td class="num">${tdLink(p.avg_days,"production","avg_production_days","days",{expert:e.name,product:p.name})}</td><td class="num">${tdLink(p.within_norm_pct,"production","within_norm_pct","pct",{expert:e.name,product:p.name})}</td><td class="num"><button class="mini-edit" data-edit-expert-product-plan="1" data-expert="${attr(e.name)}" data-product="${attr(p.name)}">План</button></td></tr>`}).join("")}</tbody></table></div></details></div>`}).join("");
  const dormant=state.production.dormant||{},reasonCards=`<div class="kpi-grid compact-cards reason-coverage-cards">${card("С причиной — все зависшие",p.dormant_all_with_reason_count||0,"production","dormant_all_with_reason_count","num",{},money(p.dormant_all_with_reason_amount||0))}${card("С причиной — выбранный период",p.dormant_with_reason_count||0,"production","dormant_with_reason_count","num",{},money(p.dormant_with_reason_amount||0))}${card("Причина заполнена",p.dormant_with_reason_pct,"production","dormant_with_reason_pct","pct",{},`${fmt(dormant.with_reason_count||0)} из ${fmt(p.dormant_count)} сделок`)}</div>`;
  const reasonsBlock=`<section class="expert-stuck-reasons"><div class="section-title">Причины зависания</div>${reasonCards}${panel("Причины по всем активным зависшим",reasonTable(dormant.all_reasons||[],"dormant_all_with_reason_count"),"Берутся из поля причины зависания в Bitrix. Нажмите на цифру, чтобы открыть сделки.")}</section>`;
...
function reasonTable(rows=state.production.dormant.reasons,metric="dormant_count"){return `<table><thead><tr><th>Причина зависания</th><th class="num">Кол-во</th><th class="num">%</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.name)}</td><td class="num">${tdLink(r.count,"production",metric,"num",{reason:r.name})}</td><td class="num">${tdLink(r.pct,"production",metric,"pct",{reason:r.name})}</td></tr>`).join("")||"<tr><td colspan='3' class='empty'>Причины пока не заполнены</td></tr>"}</tbody></table>`}
