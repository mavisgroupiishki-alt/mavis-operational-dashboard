...
let taskWorkspaceFilters={project:"",executor:"",responsible:"",deadline:"",status:"",backlog:""};
let taskEditingId="";
...
function taskProfileName(person){if(!person)return "Не назначен";return `${person.name}${person.dismissed?" · Уволен":""}`}
...
function taskOptions(rows,selected,label="Все") {return `<option value="">${esc(label)}</option>${rows.map(row=>`<option value="${attr(row.id)}" ${row.id===selected?"selected":""}>${esc(row.name)}${row.archived?" · архив":""}</option>`).join("")}`}
function filteredWorkspaceTasks(rows=keyTasksData?.tasks||[]){const f=taskWorkspaceFilters;const today=new Date().toISOString().slice(0,10);return rows.filter(task=>{
  ...
  if(f.backlog==="yes"&&!task.backlog)return false;
  if(f.backlog==="no"&&task.backlog)return false;
  return true;
})}
function taskCard(task){const people=(task.executors||[]).map(taskProfileName).join(", ")||"Не назначен";return `<button type="button" class="task-card ${task.is_overdue?"is-overdue":""} ${task.backlog?"is-backlog":""}" data-task-open="${attr(task.id)}" aria-label="Открыть задачу: ${attr(task.title)}"><span class="task-card-top"><span class="task-status-dot ${attr(task.status)}"></span><span>${esc(task.project?.name||"Без проекта")}</span>${task.priority==="high"?"<b>Высокий</b>":""}</span><strong>${esc(task.title)}</strong>${task.description?`<span class="task-card-description">${esc(task.description)}</span>`:""}<span class="task-card-footer"><span>${esc(people)}</span><time datetime="${attr(task.deadline||"")}">${task.backlog?"Бэклог":esc(taskFormatDeadline(task.deadline))}</time></span>${task.recurrence!=="none"?`<span class="task-repeat-badge">↻ ${esc(task.recurrence_label||"Повторяется")}</span>`:""}</button>`}
function taskBoard(tasks){const statuses=keyTasksData?.statuses||[];return `<div class="task-kanban">${statuses.map(status=>{const rows=tasks.filter(task=>task.status===status.id);return `<section class="task-column"><header><span class="task-status-dot ${attr(status.id)}"></span><h3>${esc(status.name)}</h3><b>${fmt(rows.length)}</b></header><div class="task-column-list">${rows.map(taskCard).join("")||'<div class="task-column-empty">Нет задач</div>'}</div></section>`}).join("")}</div>`}
function taskList(tasks){return `<div class="task-table-wrap"><table class="task-table"><thead><tr><th>Задача</th><th>Проект</th><th>Исполнители</th><th>Ответственный</th><th>Статус</th><th>Срок</th></tr></thead><tbody>${tasks.map(task=>`<tr><td><button type="button" class="task-list-open" data-task-open="${attr(task.id)}">${esc(task.title)}</button>${task.priority==="high"?'<span class="task-priority">Высокий</span>':""}</td><td>${esc(task.project?.name||"Без проекта")}</td><td>${esc((task.executors||[]).map(taskProfileName).join(", ")||"—")}</td><td>${esc(taskProfileName(task.responsible))}</td><td><span class="task-status-chip ${attr(task.status)}">${esc(task.status_label)}</span></td><td class="${task.is_overdue?"task-overdue":""}">${esc(taskFormatDeadline(task.deadline))}</td></tr>`).join("")||'<tr><td colspan="6" class="empty">По этим фильтрам задач нет.</td></tr>'}</tbody></table></div>`}
...
function taskWorkspaceControls(tasks,project){const profiles=activeTaskProfiles(),statuses=keyTasksData?.statuses||[];const profile=currentTaskProfile(),overdue=(tasks||[]).filter(task=>task.is_overdue);const reminder=overdue.length?`<button type="button" class="task-overdue-reminder" data-task-overdue-filter="1"><span>Просроченные задачи проекта</span><strong>${fmt(overdue.length)}</strong><small>Показать список →</small></button>`:"";return `<header class="task-workspace-head"><div><button class="task-back-link" type="button" data-task-project-back="1">← Все проекты</button><div class="eyebrow">ПРОЕКТ КОМАНДЫ</div><h2>${esc(project?.name||"Без проекта")}</h2><p>Задачи проекта: сроки, исполнители, бэклог и история изменений.</p></div><div class="task-workspace-actions"><button class="btn ghost" data-task-profile-open="1">${profile?`Профиль: ${esc(profile.name)}`:"Выбрать имя"}</button><button class="btn ghost" data-task-templates-open="1">Шаблоны</button><button class="btn primary" data-key-task-add="1">+ Задача</button></div></header>${reminder}<div class="task-filterbar"><select data-task-filter="executor">${taskOptions(profiles,taskWorkspaceFilters.executor,"Все исполнители")}</select><select data-task-filter="responsible">${taskOptions(profiles,taskWorkspaceFilters.responsible,"Все ответственные")}</select><select data-task-filter="deadline"><option value="">Все сроки</option><option value="overdue" ${taskWorkspaceFilters.deadline==="overdue"?"selected":""}>Просроченные</option><option value="today" ${taskWorkspaceFilters.deadline==="today"?"selected":""}>Сегодня</option><option value="week" ${taskWorkspaceFilters.deadline==="week"?"selected":""}>Ближайшие 7 дней</option><option value="none" ${taskWorkspaceFilters.deadline==="none"?"selected":""}>Без срока</option></select><select data-task-filter="backlog"><option value="">Все задачи</option><option value="yes" ${taskWorkspaceFilters.backlog==="yes"?"selected":""}>Только бэклог</option><option value="no" ${taskWorkspaceFilters.backlog==="no"?"selected":""}>Без бэклога</option></select><select data-task-filter="status"><option value="">Все статусы</option>${statuses.map(row=>`<option value="${attr(row.id)}" ${taskWorkspaceFilters.status===row.id?"selected":""}>${esc(row.name)}</option>`).join("")}</select><button class="task-filter-reset" type="button" data-task-filter-reset="1">Сбросить</button></div><div class="task-view-tabs" role="tablist"><button class="${taskWorkspaceView==="board"?"active":""}" data-task-view="board" role="tab">Канбан <b>${fmt(tasks.length)}</b></button><button class="${taskWorkspaceView==="list"?"active":""}" data-task-view="list">Список</button><button class="${taskWorkspaceView==="gantt"?"active":""}" data-task-view="gantt">Гант</button></div>`}
...
function renderTaskProjectHub(){const target=$("#key-tasks");if(!target)return;const profile=currentTaskProfile(),summaries=(keyTasksData?.project_summaries||[]).filter(project=>!project.archived),workload=(keyTasksData?.workload||[]).filter(row=>row.active);target.innerHTML=`<section class="key-tasks-page task-project-hub"><header class="task-workspace-head"><div><div class="eyebrow">ФОКУС КОМАНДЫ</div><h2>Задачи</h2><p>Выберите проект, чтобы открыть Канбан, список и Гант. Всё сохраняется в дашборде отдельно от Bitrix.</p></div><div class="task-workspace-actions"><button class="btn ghost" data-task-profile-open="1">${profile?`Профиль: ${esc(profile.name)}`:"Выбрать имя"}</button><button class="btn ghost" data-task-templates-open="1">Шаблоны</button><button class="btn ghost" data-task-projects-open="1">Управлять проектами</button><button class="btn primary" data-task-projects-open="1">+ Проект</button></div></header><section class="task-project-directory"><div class="task-directory-heading"><div><h3>Проекты</h3><p>Откройте проект, чтобы работать с его задачами.</p></div><span>${fmt(summaries.length)} активных</span></div><div class="task-project-grid">${summaries.map(project=>`<button type="button" class="task-project-tile ${taskProjectTone(project.id)}" data-task-project-open="${attr(project.id)}"><span>Проект</span><strong>${esc(project.name)}</strong><div><b>${fmt(project.active_count)}</b><small>в работе</small></div><footer><span>Бэклог ${fmt(project.backlog_count)}</span><span class="${project.overdue_count?"has-overdue":""}">${project.overdue_count?`Просрочено ${fmt(project.overdue_count)}`:"Без просрочек"}</span></footer><i>Открыть →</i></button>`).join("")||'<div class="tasks-empty-state"><strong>Добавьте первый проект</strong><span>После этого он появится здесь отдельной плиткой.</span></div>'}</div></section><section class="task-workload"><div class="task-directory-heading"><div><h3>Нагрузка команды</h3><p>Открытые задачи распределены по исполнителям.</p></div></div><div class="task-workload-grid">${workload.map(row=>`<article class="task-workload-card"><span>${esc(row.name)}</span><strong>${fmt(row.open_count)}</strong><small>в работе</small><footer><span>Бэклог ${fmt(row.backlog_count)}</span><span class="${row.overdue_count?"has-overdue":""}">Просрочено ${fmt(row.overdue_count)}</span></footer></article>`).join("")||'<div class="empty">Добавьте сотрудников, чтобы увидеть нагрузку.</div>'}</div></section></section>`;if(!currentTaskProfile()&&!taskProfilePrompted){taskProfilePrompted=true;requestAnimationFrame(openTaskProfileDialog)}}
function renderKeyTasks(){
  if(!activeTaskProfileId){try{activeTaskProfileId=localStorage.getItem(TASK_PROFILE_KEY)||""}catch(e){}}
  if(taskWorkspaceScreen!=="project"){renderTaskProjectHub();return}
...
function openTaskProfileDialog(){const profiles=activeTaskProfiles();$("#taskProfileChoices").innerHTML=profiles.map(row=>`<span class="task-profile-choice ${row.id===activeTaskProfileId?"active":""}"><button data-task-profile-select="${attr(row.id)}">${esc(row.name)}</button><button class="task-profile-delete" data-task-profile-delete="${attr(row.id)}" aria-label="Удалить ${attr(row.name)}">×</button></span>`).join("")||'<span class="muted">Добавьте первое имя.</span>';$("#taskProfileName").value="";$("#taskProfileDialog").showModal()}
...
function updateBacklogDeadlineState(){const backlog=$("#keyTaskBacklog").checked,deadline=$("#keyTaskDeadline");deadline.disabled=backlog;deadline.required=!backlog;if(backlog)deadline.value=""}
...
function openKeyTaskDialog(taskId=""){const task=(keyTasksData?.tasks||[]).find(row=>row.id===taskId);taskEditingId=task?.id||"";const profiles=activeTaskProfiles(),projects=(keyTasksData?.projects||[]).filter(row=>!row.archived||row.id===task?.project_id),statuses=keyTasksData?.statuses||[],defaultProject=task?.project_id||taskWorkspaceProjectId;$("#taskDialogTitle").textContent=task?"Редактирование задачи":"Новая задача";$("#keyTaskTemplate").innerHTML=`<option value="">Без шаблона</option>${(keyTasksData?.templates||[]).map(row=>`<option value="${attr(row.id)}">${esc(row.name)}</option>`).join("")}`;$("#keyTaskProject").innerHTML=taskOptions(projects,defaultProject,"Выберите проект");$("#keyTaskStatus").innerHTML=statuses.map(row=>`<option value="${attr(row.id)}" ${row.id===(task?.status||"new")?"selected":""}>${esc(row.name)}</option>`).join("");$("#keyTaskResponsible").innerHTML=taskOptions(profiles,task?.responsible?.id||activeTaskProfileId,"Выберите");$("#keyTaskExecutors").innerHTML=profiles.map(row=>`<option value="${attr(row.id)}" ${(task?.executors||[]).some(person=>person.id===row.id)||(!task&&row.id===activeTaskProfileId)?"selected":""}>${esc(row.name)}</option>`).join("");$("#keyTaskTitle").value=task?.title||"";$("#keyTaskDeadline").value=task?.deadline||"";$("#keyTaskPriority").value=task?.priority||"normal";$("#keyTaskRecurrence").value=task?.recurrence||"none";$("#keyTaskDescription").value=task?.description||"";$("#keyTaskBacklog").checked=Boolean(task?.backlog);updateBacklogDeadlineState();$("#deleteTaskFromDialog").hidden=!task;$("#taskActivityPanel").hidden=!task;$("#taskActivityPanel").innerHTML="";$("#keyTaskDialog").showModal();$("#keyTaskTitle").focus();if(task)loadTaskActivity(task.id)}
async function saveKeyTask(){const executor_ids=[...$("#keyTaskExecutors").selectedOptions].map(option=>option.value),backlog=$("#keyTaskBacklog").checked;const payload={title:$("#keyTaskTitle").value.trim(),responsible_id:$("#keyTaskResponsible").value,executor_ids,project_id:$("#keyTaskProject").value,deadline:$("#keyTaskDeadline").value,priority:$("#keyTaskPriority").value,status:$("#keyTaskStatus").value,recurrence:$("#keyTaskRecurrence").value,description:$("#keyTaskDescription").value.trim(),backlog,created_by_profile_id:activeTaskProfileId,changed_by_profile_id:activeTaskProfileId};const missing=!payload.title||!payload.project_id||!payload.responsible_id||!payload.executor_ids.length||!payload.priority||!payload.description||(!backlog&&!payload.deadline);if(missing){alert(backlog?"Заполните название, проект, ответственного, исполнителя, приоритет и описание.":"Заполните все обязательные поля, включая срок.");return}const url=taskEditingId?`/api/key-tasks/${encodeURIComponent(taskEditingId)}`:"/api/key-tasks";const response=await fetch(url,{method:taskEditingId?"PATCH":"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});if(!response.ok){alert((await response.json().catch(()=>({detail:"Не удалось сохранить задачу"}))).detail||"Не удалось сохранить задачу");return}$("#keyTaskDialog").close();keyTasksData=null;await loadKeyTasks(true)}
...
function init(){
  ...
  document.body.addEventListener('click',e=>{
    ...
    const taskProjectOpen=e.target.closest('[data-task-project-open]');if(taskProjectOpen){taskWorkspaceProjectId=taskProjectOpen.dataset.taskProjectOpen||"";taskWorkspaceScreen="project";taskWorkspaceFilters={project:"",executor:"",responsible:"",deadline:"",status:"",backlog:""};renderKeyTasks();return}
    if(e.target.closest('[data-task-project-back]')){taskWorkspaceScreen="projects";taskWorkspaceProjectId="";taskWorkspaceFilters={project:"",executor:"",responsible:"",deadline:"",status:"",backlog:""};renderKeyTasks();return}
    ...
    if(e.target.closest('[data-task-filter-reset]')){taskWorkspaceFilters={project:"",executor:"",responsible:"",deadline:"",status:"",backlog:""};renderKeyTasks();return}
  ...
  document.body.addEventListener('input',e=>{if(e.target?.id==='dashboardChatQuestion'){dashboardChatDraft=e.target.value;persistDashboardChatState()}});
  ...
  document.body.addEventListener('change',e=>{if(e.target.matches('[data-audit-filter]'))renderCrmAuditRows();if(e.target.matches('[data-task-filter]')){taskWorkspaceFilters[e.target.dataset.taskFilter]=e.target.value;renderKeyTasks()}if(e.target.id==='keyTaskBacklog')updateBacklogDeadlineState();if(e.target.id==='keyTaskTemplate')applyTaskTemplate(e.target.value)});
  ...
  $("#saveTaskProfile").addEventListener('click',async()=>{const response=await fetch('/api/key-tasks/profiles',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:$("#taskProfileName").value})});if(!response.ok){alert((await response.json().catch(()=>({detail:"Не удалось добавить сотрудника"}))).detail||"Не удалось добавить сотрудника");return}const payload=await response.json();saveTaskProfile(payload.profile.id);$("#taskProfileDialog").close();keyTasksData=null;await loadKeyTasks(true)});
