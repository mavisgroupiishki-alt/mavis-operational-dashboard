...
function keyTaskItem(task){return `<article class="key-task-item ${task.reason==="Просрочена"?"is-overdue":""}"><div class="key-task-item-title"><strong>${esc(task.title)}</strong><button type="button" class="key-task-delete" data-key-task-delete="${attr(task.id)}" aria-label="Удалить задачу ${attr(task.title)}">×</button></div><span class="key-task-status">${esc(task.reason||"В работе")}</span><small>${esc(keyTaskFormatDeadline(task.deadline))}${task.priority==="high"?" · высокий приоритет":""}</small></article>`}
function keyTaskPersonCard(person){const tasks=person.tasks||[];return `<article class="key-task-person"><header><div><div class="eyebrow">${esc(person.name)}</div><strong>${person.active_count?`${fmt(person.active_count)} задач`:'Нет задач на эту неделю'}</strong></div>${person.active_count>5?`<span class="key-task-limit">показаны 5</span>`:""}</header>${tasks.length?`<div class="key-task-list">${tasks.map(keyTaskItem).join("")}</div>`:'<div class="key-task-empty">На эту неделю задач нет.</div>'}</article>`}
...
function renderKeyTasks(){
  ...
  if(!members.length){target.innerHTML=`<section class="key-tasks-page"><header class="key-tasks-head"><div><div class="eyebrow">ФОКУС КОМАНДЫ</div><h2>Ключевые задачи</h2><p>Выберите сотрудников из Bitrix, затем создавайте задачи вручную прямо в дашборде.</p></div><button class="btn primary" data-key-task-team-open="1">Выбрать сотрудников</button></header><div class="key-tasks-empty-state"><strong>Пока никто не выбран</strong><span>Список сотрудников сохранится отдельно от менеджеров и экспертов.</span></div></section>`;return}
  ...
  target.innerHTML=`<section class="key-tasks-page"><header class="key-tasks-head"><div><div class="eyebrow">ФОКУС КОМАНДЫ</div><h2>Ключевые задачи</h2><p>Ручной список задач: добавляйте и удаляйте их здесь. Сотрудники выбираются из Bitrix, сами задачи в Bitrix не создаются.</p></div><div class="key-tasks-actions"><button class="btn primary" data-key-task-add="1">+ Добавить задачу</button><button class="btn ghost" data-key-task-team-open="1">Сотрудники</button></div></header><div class="key-task-weekbar"><button class="icon key-task-week-shift" data-key-task-week-shift="-1" aria-label="Предыдущая неделя">←</button><div><span>Выбранная неделя</span><strong>${esc(keyTaskWeekLabel(keyTaskWeekStart))}</strong></div><button class="btn ghost key-task-this-week" data-key-task-this-week="1">Текущая</button><button class="icon key-task-week-shift" data-key-task-week-shift="1" aria-label="Следующая неделя">→</button></div><nav class="key-task-month-weeks" aria-label="Недели выбранного месяца">${monthWeeks.map(start=>{const count=((data.weekly_people||{})[start]||[]).reduce((sum,row)=>sum+(row.active_count||0),0);return `<button class="${start===keyTaskWeekStart?'active':''}" data-key-task-week="${start}"><span>${esc(keyTaskWeekLabel(start))}</span><b>${fmt(count)}</b></button>`}).join("")}</nav><div class="key-task-people">${people.map(keyTaskPersonCard).join("")}</div>${noDeadline.length?`<details class="key-task-no-deadline"><summary><span>Без дедлайна</span><span>${fmt(noDeadline.length)} задач</span></summary><div class="key-task-list">${noDeadline.map(keyTaskItem).join("")}</div></details>`:""}</section>`;
}
async function loadKeyTasks(force=false){
  ...
  keyTasksRequest=(async()=>{try{const teamResponse=await fetch('/api/key-tasks/team',{cache:'no-store'}),team=await teamResponse.json().catch(()=>({detail:'Bitrix временно недоступен'}));if(!teamResponse.ok)throw new Error(team.detail||'Не удалось получить список сотрудников');keyTaskTeamData=team;const response=await fetch('/api/key-tasks',{cache:'no-store'}),payload=await response.json().catch(()=>({detail:'Сервис задач временно недоступен'}));if(!response.ok||!payload.ok)throw new Error(payload.detail||payload.error||'Не удалось загрузить задачи');keyTasksData=payload;renderKeyTasks()}catch(error){renderKeyTasksPlaceholder(error.message||'Задачи временно недоступны')}finally{keyTasksRequest=null}})();return keyTasksRequest;
}
...
function openKeyTaskDialog(){const members=keyTasksData?.members||keyTaskTeamData?.members||[];$("#keyTaskResponsible").innerHTML=members.map(row=>`<option value="${attr(row.id)}">${esc(row.name)}</option>`).join("");$("#keyTaskTitle").value="";$("#keyTaskDeadline").value=keyTaskWeekStart||defaultKeyTaskWeek();$("#keyTaskPriority").value="normal";$("#keyTaskDialog").showModal();$("#keyTaskTitle").focus()}
async function saveKeyTask(){const payload={title:$("#keyTaskTitle").value,responsible_id:$("#keyTaskResponsible").value,deadline:$("#keyTaskDeadline").value,priority:$("#keyTaskPriority").value};const response=await fetch('/api/key-tasks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!response.ok){alert(await response.text());return}$("#keyTaskDialog").close();keyTasksData=null;await loadKeyTasks(true)}
...
function init(){
  ...
  document.body.addEventListener('click',e=>{
    ...
    if(e.target.closest('[data-key-task-add]')){openKeyTaskDialog();return}
    const keyTaskDelete=e.target.closest('[data-key-task-delete]');if(keyTaskDelete){deleteKeyTask(keyTaskDelete.dataset.keyTaskDelete);return}
