...
function taskWorkspaceControls(tasks){const profiles=activeTaskProfiles(),projects=keyTasksData?.projects||[],statuses=keyTasksData?.statuses||[];const profile=currentTaskProfile();return `<header class="task-workspace-head"><div><div class="eyebrow">ФОКУС КОМАНДЫ</div><h2>Задачи</h2><p>Ручной рабочий список. Он живёт в дашборде и не создаёт задачи в Bitrix.</p></div><div class="task-workspace-actions"><button class="btn ghost" data-task-profile-open="1">${profile?`Профиль: ${esc(profile.name)}`:"Выбрать имя"}</button><button class="btn ghost" data-task-projects-open="1">Проекты</button><button class="btn primary" data-key-task-add="1">+ Задача</button></div></header><div class="task-filterbar"><select data-task-filter="project">${taskOptions(projects,taskWorkspaceFilters.project,"Все проекты")}</select><select data-task-filter="executor">${taskOptions(profiles,taskWorkspaceFilters.executor,"Все исполнители")}</select><select data-task-filter="responsible">${taskOptions(profiles,taskWorkspaceFilters.responsible,"Все ответственные")}</select><select data-task-filter="deadline"><option value="">Все сроки</option><option value="overdue" ${taskWorkspaceFilters.deadline==="overdue"?"selected":""}>Просроченные</option><option value="today" ${taskWorkspaceFilters.deadline==="today"?"selected":""}>Сегодня</option><option value="week" ${taskWorkspaceFilters.deadline==="week"?"selected":""}>Ближайшие 7 дней</option><option value="none" ${taskWorkspaceFilters.deadline==="none"?"selected":""}>Без срока</option></select><select data-task-filter="status"><option value="">Все статусы</option>${statuses.map(row=>`<option value="${attr(row.id)}" ${taskWorkspaceFilters.status===row.id?"selected":""}>${esc(row.name)}</option>`).join("")}</select><button class="task-filter-reset" type="button" data-task-filter-reset="1">Сбросить</button></div><div class="task-view-tabs" role="tablist"><button class="${taskWorkspaceView==="board"?"active":""}" data-task-view="board" role="tab">Канбан <b>${fmt(tasks.length)}</b></button><button class="${taskWorkspaceView==="list"?"active":""}" data-task-view="list" role="tab">Список</button><button class="${taskWorkspaceView==="gantt"?"active":""}" data-task-view="gantt" role="tab">Гант</button></div>`}
