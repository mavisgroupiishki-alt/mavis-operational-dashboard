...
let taskWorkspaceFilters={project:"",executor:"",responsible:"",deadline:"",status:""};
let taskEditingId="";
...
function filteredWorkspaceTasks(){const f=taskWorkspaceFilters;const today=new Date().toISOString().slice(0,10);return (keyTasksData?.tasks||[]).filter(task=>{
...
function taskCard(task){const people=(task.executors||[]).map(taskProfileName).join(", ")||"Не назначен";return `<button type="button" class="task-card ${task.is_overdue?"is-overdue":""}" data-task-open="${attr(task.id)}" aria-label="Открыть задачу: ${attr(task.title)}"><span class="task-card-top"><span class="task-status-dot ${attr(task.status)}"></span><span>${esc(task.project?.name||"Без проекта")}</span>${task.priority==="high"?"<b>Высокий</b>":""}</span><strong>${esc(task.title)}</strong>${task.description?`<span class="task-card-description">${esc(task.description)}</span>`:""}<span class="task-card-footer"><span>${esc(people)}</span><time datetime="${attr(task.deadline||"")}">${esc(taskFormatDeadline(task.deadline))}</time></span>${task.recurrence!=="none"?`<span class="task-repeat-badge">↻ ${esc(task.recurrence_label||"Повторяется")}</span>`:""}</button>`}
...
function openKeyTaskDialog(taskId=""){const task=(keyTasksData?.tasks||[]).find(row=>row.id===taskId);taskEditingId=task?.id||"";const profiles=activeTaskProfiles(),projects=keyTasksData?.projects||[],statuses=keyTasksData?.statuses||[];$("#taskDialogTitle").textContent=task?"Редактирование задачи":"Новая задача";$("#keyTaskProject").innerHTML=taskOptions(projects,task?.project_id||"","Без проекта");$("#keyTaskStatus").innerHTML=statuses.map(row=>`<option value="${attr(row.id)}" ${row.id===(task?.status||"new")?"selected":""}>${esc(row.name)}</option>`).join("");$("#keyTaskResponsible").innerHTML=taskOptions(profiles,task?.responsible?.id||activeTaskProfileId,"Выберите");$("#keyTaskExecutors").innerHTML=profiles.map(row=>`<option value="${attr(row.id)}" ${(task?.executors||[]).some(person=>person.id===row.id)?"selected":""}>${esc(row.name)}</option>`).join("");$("#keyTaskTitle").value=task?.title||"";$("#keyTaskDeadline").value=task?.deadline||"";$("#keyTaskPriority").value=task?.priority||"normal";$("#keyTaskRecurrence").value=task?.recurrence||"none";$("#keyTaskDescription").value=task?.description||"";$("#deleteTaskFromDialog").hidden=!task;$("#taskActivityPanel").hidden=!task;$("#taskActivityPanel").innerHTML="";$("#keyTaskDialog").showModal();$("#keyTaskTitle").focus();if(task)loadTaskActivity(task.id)}
async function saveKeyTask(){const executor_ids=[...$("#keyTaskExecutors").selectedOptions].map(option=>option.value);const payload={title:$("#keyTaskTitle").value,responsible_id:$("#keyTaskResponsible").value,executor_ids,project_id:$("#keyTaskProject").value,deadline:$("#keyTaskDeadline").value,priority:$("#keyTaskPriority").value,status:$("#keyTaskStatus").value,recurrence:$("#keyTaskRecurrence").value,description:$("#keyTaskDescription").value,created_by_profile_id:activeTaskProfileId,changed_by_profile_id:activeTaskProfileId};if(!payload.responsible_id){alert("Выберите ответственного");return}const url=taskEditingId?`/api/key-tasks/${encodeURIComponent(taskEditingId)}`:"/api/key-tasks";const response=await fetch(url,{method:taskEditingId?"PATCH":"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});if(!response.ok){alert((await response.json().catch(()=>({detail:"Не удалось сохранить задачу"}))).detail||"Не удалось сохранить задачу");return}$("#keyTaskDialog").close();keyTasksData=null;await loadKeyTasks(true)}
...
function init(){
  ...
  document.body.addEventListener('click',e=>{
    ...
    if(e.target.closest('[data-task-filter-reset]')){taskWorkspaceFilters={project:"",executor:"",responsible:"",deadline:"",status:""};renderKeyTasks();return}
  ...
  document.body.addEventListener('change',e=>{if(e.target.matches('[data-audit-filter]'))renderCrmAuditRows();if(e.target.matches('[data-task-filter]')){taskWorkspaceFilters[e.target.dataset.taskFilter]=e.target.value;renderKeyTasks()}});
